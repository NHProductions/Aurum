...
void rRandom(auFunc) {
    ...
    if (strcmp(identifier, "rand") == 0) List_InsertElement(vms->stack, 0, numToTV((num){.type = NUM_INT, .value.iVal = rand()}));
    if (strcmp(identifier, "randL") == 0) {
        ...
        List_InsertElement(vms->stack, 0, numToTV((num){.type = NUM_LONG, .value.lVal = (rand64() % (max - min + 1)) + min}));
        freeTypedValue(arg0);
        freeTypedValue(arg1);
    }
    if (strcmp(identifier, "randLD") == 0) {
        ...
        List_InsertElement(vms->stack, 0, numToTV((num){.type = NUM_LONGDOUBLE, .value.ldVal = min + ((long double)rand() / RAND_MAX) * (max - min)}));
        freeTypedValue(arg0);
        freeTypedValue(arg1);
    }
    if (strcmp(identifier, "factorial") == 0 || strcmp(identifier, "gamma") == 0 || strcmp(identifier, "lgamma") == 0) {
        ...
        if (strcmp(identifier, "factorial") == 0) List_InsertElement(vms->stack, 0, numToTV((num){.type = NUM_LONG, .value.lVal = (int64_t)tgammal(n)}));
        if (strcmp(identifier, "gamma") == 0 ) List_InsertElement(vms->stack, 0, numToTV((num){.type = NUM_LONGDOUBLE, .value.ldVal = tgammal(n)}));
        if (strcmp(identifier, "lgamma") == 0 ) List_InsertElement(vms->stack, 0, numToTV((num){.type = NUM_LONGDOUBLE, .value.ldVal = lgammal(n)}));
        freeTypedValue(arg0);
    }
    if (strcmp(identifier, "permutations") == 0 || strcmp(identifier, "combinations") == 0) {
        ...
        if (isPermut && doRepetition) List_InsertElement(vms->stack, 0, numToTV(convertNum((num){.type = NUM_LONGDOUBLE, .value.ldVal = powl((long double)n, (long double)r)}, arg0->value.numberValue.type)));
        if (isPermut && !doRepetition) List_InsertElement(vms->stack, 0, numToTV(convertNum((num){.type = NUM_LONGDOUBLE, .value.ldVal = (tgammal(n+1))/(tgammal(1+n-r))}, arg0->value.numberValue.type)));
        if (!isPermut && doRepetition) List_InsertElement(vms->stack, 0, numToTV(convertNum((num){.type = NUM_LONGDOUBLE, .value.ldVal = (tgammal(n+r)/(tgammal(r)*tgammal(n)))}, arg0->value.numberValue.type)));
        if (!isPermut && !doRepetition) List_InsertElement(vms->stack, 0, numToTV(convertNum((num){.type = NUM_LONGDOUBLE, .value.ldVal = (tgammal(n+1))/(tgammal(r+1)*tgammal(n-r+1))}, arg0->value.numberValue.type)));
