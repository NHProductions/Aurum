...
struct List_Node* List_GetElement(List* L, int Index) {
    if (Index == -1) {
        ...
        struct List_Node* Current_Data = L->First;
        if (Index >= L->length) {
            printf("List Access Error! Tried to access %d, when list has %d elements.", Index, L->length);    
            return NULL;
        }
...
void pushStack(List* stack, void* data) {
    List_InsertElement(stack, 0, data);
}
void* popStack(List* stack) {
    struct List_Node *old = stack->First;
    void *data = old->data;
...
void Map_SetKey(Hashmap* HM, void* key, void* value) {
    ...
    bool Found = false;
    for (int i = 0; i < N->length; i++) {
        if ( HM->comp(((Hashmap_Entry*)List_GetElement(N,i)->data)->Key, key) == 0) {
