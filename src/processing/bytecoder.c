...
#include "../systemLibraries/mathFunctions.h"
#include "../systemLibraries/timeFunctions.h"
#include "../systemLibraries/randomFunctions.h"
...
structDefinition* getStructType(ASTNode* currentNode, byteCode c, scopeInfo s) {
    ...
    // n.a[0]
        ...
        if (strcmp(currentNode->value.opVal, "&") == 0 && currentNode->left == NULL) {
            // Check if there's a function already declared whose name is it.
            structDefinition* toReturn =  getStructDefViaName("fnptr", c);
            if (toReturn == NULL) fatalError(0x41, "Unary & requires @fn to be imported.", -1);
            return toReturn;
        }
    ...
    switch (currentNode->type) {
        ...
        */
        case AST_IDENTIFIER: {
            ...
            for (int i = 0; i < c.functionIdentifiers->length; i++) {
                char* fName = ((bcFunction*)getArray(c.functionIdentifiers, i))->name;
                if (strcmp(fName, name) == 0) {
                    addUnaryInstruction(c, OP_FPTR, i, chunkIdx);
                    return;
                }
            }
        ...
        case AST_OPERATOR: {
            ...
            if (currentNode->right != NULL) toBytecode(c, currentNode->right, s, chunkIdx);
                ...
                if (left == NULL && right == NULL) {
                    ...
                    else if (strcmp(currentNode->value.opVal, "&") == 0 && currentNode->left != NULL) addNullaryInstruction(c, OP_BAND, chunkIdx);
                    //else if (strcmp(currentNode->value.opVal, "&") == 0 && currentNode->left == NULL) addNullaryInstruction(c, OP_FPTR, chunkIdx);
        ...
        */
        case AST_STRUCTACCESS: {
            if (currentNode->left->type == AST_IDENTIFIER) {
                ...
                char toSearch[50]; sprintf(toSearch, "%s_%s", sd->name, currentNode->right->value.nameVal);
                for (int i = 0; i < c.functionIdentifiers->length; i++) {
                    bcFunction* bcf = getArray(c.functionIdentifiers, i);
                    if (strcmp(bcf->name, toSearch) == 0) {
                        addUnaryInstruction(c, OP_FPTR, i, chunkIdx);
                        return;
                    }
                }
        ...
        case AST_VARDECLARATION: {
            ...
            */
            ...
            if (currentNode->left->type == AST_DATATYPE && currentNode->left->left == NULL) {
                ...
                // Check if it's some literal (such as a function pointer)
                structDefinition* child0Def = getStructType(List_GetElement(currentNode->children, 0)->data, c, s);
                if (child0Def != NULL) {
                    if (strcmp(child0Def->name, "fnptr") == 0) {
                        toBytecode(c, List_GetElement(currentNode->children, 0)->data, s, chunkIdx);
                        vmVariable* toApp = malloc(sizeof(vmVariable));
                        *toApp = (vmVariable){
                            .isArray = false,
                            .name = strdup(currentNode->right->value.nameVal),
                            .structType = strdup("fnptr"),
                            .t = TYPE_STRUCT
                        };
                        applySpecifiers(currentNode, toApp);
                        if (chunkIdx == 0) {appendArray(c.globals, toApp); addUnaryInstruction(c, OP_STORE_GLOBAL, c.globals->length-1, chunkIdx);}
                        else {List_AppendElement(s.locals, toApp); addUnaryInstruction(c, OP_STORE_LOCAL, s.locals->length-1, chunkIdx);}
                        return;
                    }
                ...
                if (isDebug) printAST(currentNode, 0);
        ...
        */
        ...
        case AST_FUNCCALL: {
            ...
            // check if the left is a ptr
            structDefinition* leftStruct = getStructType(currentNode->left, c, s);
            bool dcall = false;
             
            if (leftStruct != NULL) {if (strcmp(leftStruct->name, "fnptr") == 0) dcall = true;}
            if (dcall) {
                for (int i = 0; i < currentNode->children->length; i++) {
                    ASTNode* param = List_GetElement(currentNode->children, i)->data;
                    toBytecode(c, param->left, s, chunkIdx);
                }
                toBytecode(c, currentNode->left, s, chunkIdx);
                addUnaryInstruction(c, OP_DCALL, currentNode->children->length, chunkIdx);
                break;
            }
...
void printBytecode(byteCode c, bool numsOnly, bool numberInstructions) {
    ...
    for (int i = 0; i < c.constants->length; i++) {
        ...
        if (tv->valueType == TYPE_NUM) {
            ...
            if (tv->value.av.arrayType != AT_UNKNOWN && tv->value.av.data != NULL && tv->value.av.len > 0) {
