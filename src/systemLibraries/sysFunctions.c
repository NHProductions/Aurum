...
short cmpFunc = 0;
virtualMachineState* cvms = NULL;
int cmpTV(const void* av, const void* bv) {
    typedValue* a = *(typedValue**)av;
    typedValue* b = *(typedValue**)bv;

    array* params = mallocArray(0);
    stackVariable* asv = malloc(sizeof(stackVariable));
    stackVariable* bsv = malloc(sizeof(stackVariable));
    *asv = *bsv = (stackVariable){
        .name = NULL,
    };
    asv->structType = a->valueType == TYPE_STRUCT ? strdup(a->value.so.def->name) : NULL;
    bsv->structType = b->valueType == TYPE_STRUCT ? strdup(b->value.so.def->name) : NULL;
    if (a->valueType == TYPE_ARRAY) {asv->structType = a->value.av.arrayType == AT_CHARARR ? strdup("string") : strdup("array");}
    if (b->valueType == TYPE_ARRAY) {bsv->structType = b->value.av.arrayType == AT_CHARARR ? strdup("string") : strdup("array");}
    asv->t = a->valueType;
    bsv->value = deepcopyTypedValue(b);
    asv->value = deepcopyTypedValue(a);
    asv->isArray = a->valueType == TYPE_ARRAY;
    bsv->isArray = b->valueType == TYPE_ARRAY;
    asv->name = NULL;
    bsv->name = NULL;
    appendArray(params, asv);
    appendArray(params, bsv);
    
    array* newStack = mallocArray(0);

    virtualMachineState* temp = malloc(sizeof(virtualMachineState));
    temp->bc = cvms->bc;
    temp->globals = cvms->globals;
    temp->stack = newStack;

    bcFunction* toCall = getArray(cvms->bc->functionIdentifiers, cmpFunc);
    chunk* n = NULL;
    int chunkidx = 0;
    for (int i = 0; i < cvms->bc->chunks->length; i++) {
        if (strcmp(toCall->name, ((chunk*)getArray(cvms->bc->chunks, i))->name ) == 0 ) {chunkidx = i; n = (chunk*)getArray(cvms->bc->chunks, i); break;}
    }
    if (n == NULL) fatalError(0x30, "", -1);
    bool doBreak = false;
    for (int i = 0; i < n->instructions->length && !doBreak; i++) {
        int prevIVal = i; 
        executeLine(temp, chunkidx, &i, params, &doBreak);
        if (i != prevIVal) {i--;}
    }
    typedValue* returnValue = popArray(newStack);
    num r = convertNum(returnValue->value.numberValue, NUM_LONG);
    
    freeTypedValue(asv->value);
    free(asv->structType);
    free(asv);
    freeTypedValue(bsv->value);
    free(bsv->structType);
    free(bsv);
    for (int i = 0; i < newStack->length; i++) {
        freeTypedValue(getArray(newStack, i));
    }
    freeArray(newStack, false);
    freeTypedValue(returnValue);
    free(temp);
    return r.value.lVal;
}
...
void sSort(auFunc) {
    ...
    typedValue* arg1 = getArray(args,  1);
    if (arg0->valueType != TYPE_ARRAY) {fatalError(0x30, "Invalid input into a system function.", -1);}
    if (arg1->valueType != TYPE_STRUCT) {fatalError(0x30, "Invalid input into a system function.", -1);}
    if (strcmp(arg1->value.so.def->name, "fnptr") != 0) {fatalError(0x30, "Invalid input into a system function.", -1);}
    cmpFunc = arg1->value.so.fields[0]->value.numberValue.value.sVal;
    cvms = vms;
    bool r = convertNum(arg1->value.numberValue, NUM_BOOL).value.bVal;
    qsort(arg0->value.av.data, arg0->value.av.len, sizeof(typedValue*), cmpTV);
    cmpFunc = 0;
    cvms = NULL;
    freeTypedValue(arg1);
    pushArray(vms->stack, arg0);
}
...
void systemCall(bcFunction* bcDef, virtualMachineState* vms, int argc) {
    ...
    // @time
    else if (strcmp(identifier, "time") == 0 || strcmp(identifier, "clock") == 0 || strcmp(identifier, "getUTC") == 0 || strcmp(identifier, "toUTC") == 0 || strcmp(identifier, "sleep") == 0 || strcmp(identifier, "timeld") == 0) tTime(auFuncCall);
    // @rand
