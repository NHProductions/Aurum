...
void tTime(auFunc) {
    ...
    if (strcmp(identifier, "timeld") == 0) {
        struct timespec ts;
        if (timespec_get(&ts, TIME_UTC)) {
            long double toReturn = (long double)ts.tv_sec;
            toReturn += (long double)ts.tv_nsec/(long double)(1000*1000*1000);
            pushArray(vms->stack,numToTV((num){.type = NUM_LONGDOUBLE, .value.ldVal = toReturn})); return;
