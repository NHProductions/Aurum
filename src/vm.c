...
void executeLine(virtualMachineState* vms, int chunkIdx, int* line, List* locals, bool* returnLn) {
    ...
    switch (currentInstruction->code) {
        ...
        case OP_LOAD_LOCAL: {
            stackVariable* sv = (stackVariable*)List_GetElement(locals, currentInstruction->args[0])->data;
            typedValue* A = deepcopyTypedValue(sv->value);
            A->ptr = malloc(sizeof(stackPtr));
            *((stackPtr*)A->ptr) = (stackPtr){
                .addr = sv,
        ...
        */
        case OP_CALL: {
            ...
            if (bcf->isSystem) {
                ...
                List* toSend = malloc(sizeof(List));
                *toSend = NewList();
                for (int i = 0; i < currentInstruction->args[1]; i++) {
                    ...
                    *sv = (stackVariable){
                    ...
                    List_InsertElement(toSend, 0, sv);
                }
                chunk* toCall = NULL;
                int cidx;
