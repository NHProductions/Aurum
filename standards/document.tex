...
\begin{document}
	...
	\section{Getting started}
		\subsubsection{Requirements}
			\begin{itemize}
				...
				\item[--] Atleast 819kb of free storage.
		...
		\begin{enumerate}
			...
			\item Now, you're able to run Aurum files by doing \spverb|aurum file.aur|.
		\end{enumerate}
	...
	\begin{itemize}
		...
		\end{spverbatim}
		\newpage 
		\item[--] \spverb|array_sort(bool) -> void| - Sorts an array. The array has to be an array of numbers. If the boolean passed in is true, sorts from smallest to largest. If it's false, sorts from largest to smallest. Modifies the original array. Examples:
		
		
		\begin{spverbatim}
			int[] arr = [4,3,5,4,7,1,9,10];
			arr.sort(true);
			print("\n%a\n", arr); ` Prints [1,3,4,4,5,7,9,10]
			arr.sort(false);
			print("%a\n", arr); ` Prints [10,9,7,5,4,4,3,1]
		\end{spverbatim}
	\end{itemize}
	...
	\begin{itemize}
		...
		\item[--] \spverb|cplxNum_op_add(cplxNum) -> cplxNum| - Adds two complex numbers.
		\item[--] \spverb|cplxNum_op_sub(cplxNum) -> cplxNum| - Subtracts two complex numbers.
		\item[--] \spverb|cplxNum_op_mul(cplxNum) -> cplxNum| - Multiplies two complex numbers.
		\newline
		\begin{spverbatim}
	...
	\begin{itemize}
		...
		\item[--] \spverb|vector?_op_add(vector?) -> vector?| - Adds two vectors.
		\item[--] \spverb|vector?_op_sub(vector?) -> vector?| - Subtracts two vectors.
	...
	\begin{itemize}
		...
		\begin{spverbatim}
			[[11, 12, 13],
			 [14, 15, 16]]
		\end{spverbatim}
		\item[--] \spverb|mtrx_mult(mtrx) -> mtrx| - Multiplies two matrices. If they are unable to be multiplied, throws an error.
		...
		\item[--] \spverb|mtrx_op_add(mtrx) -> mtrx| - Adds two matrices
		\item[--] \spverb|mtrx_op_sub(mtrx) -> mtrx| - Subtracts two matrices.
	\end{itemize}
	...
	\begin{itemize}
		\item[--] Speed optimizations --- The language is extremely slow right now.
		\item[--] Operator overloading --- operator overloads will be declared in the form of op\_add(), op\_sub, and so on. This will then allow that operator to interact with structs.
