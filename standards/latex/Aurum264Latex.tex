...
\begin{document}
	...
	\section{Changed from Last Version}
		New Features:
		\begin{itemize}
			\item[--] \spverb|clrcon() -> void| --- Clears the console that the program is running in.
			\item[--] \spverb|dimcon() -> int[]| --- Gets the current dimensions of the console the program is running in. 
			\item[--] \spverb|popcon(bool, bool) -> char| --- Pops a character from the console. If arg0 is true, then returns the last character. If arg1 is true, then removes that character from the console.
		\end{itemize}
		Miscellaneous Changes:
		\begin{itemize}
			\item[--] Fixed bug where time functions wouldn't free their arguments correctly, causing a memory leak.
			\item[--] Fixed bug where string concatenation, string equality, and string length wouldn't work correctly due to being mistakenly parsed as operator overloads.
	...
	\begin{itemize}
		...
		\end{spverbatim}
		\newline
		\item[--] \spverb|dimcon() -> int[]| - Gets the current dimensions of the console the program is running in. Returns it in the format of [cols, rows].
		\item[--] \spverb|clrcon() -> void| - Clears the console the program is running in.
		\item[--] \spverb|popcon(bool, bool) -> char| --- Pops a character from the console. If arg0 is true, then returns the last character. If arg1 is true, then removes that character from the console.
		\newpage
	\end{itemize}
	...
	\subsubsection{Directories}
		\begin{itemize}
			...
			\end{spverbatim}
		\end{itemize}
	\subsection{@apl Library}
		This is a library for developing windows-based applications. As such, linux or other operating systems is not compatable with this library. This library makes heavy use of the Win32 API in C.
