...
\begin{document}
	...
	\section{Changed from Last Version}
		New Features
		\begin{itemize}
			\item[--] \spverb|typeof(auto) -> string| - New function used for getting the types of things. See \ref{misc:typeof}.
			\item[--] Operator Overloading --- Methods used to add custom behavior for operators when dealing with structs. See \ref{misc:operatorOverloading} 
		\end{itemize}
		Miscellaneous Changes:
		\begin{itemize}
			\item[--] Fixed bug where nested accesses weren't converted correctly into bytecode.
			\item[--] Changed linked lists to dynamic arrays within the bytecoding \& VM stages. This change added a 4\% increase in speed when doing 100x100 matrix addition.
			\item[--] Added a pool allocator (Instead of allocating things to the heap, typedValues are allocated to an array with 1 million entries). This change added a 94\% increase in speed when doing 100x100 matrix addition.
		\end{itemize}
	\newpage
	...
	\begin{itemize}
		...
		\item[--] Function references --- More dynamic function references / more similar to C.
